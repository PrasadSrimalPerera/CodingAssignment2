 ...
 * 
 * @author prasad
 * EricssonCorporationEntry presents a data entry type from application to MongoDB Document
 *
 */
public class EricssonCorporationEntry {
	...
	public static EricssonCorporationEntry addItem(String content) {
		...
		docBuilder.append("_id", entryID);
		docBuilder.add("content", content);
		DBObject nextEntry = docBuilder.get();
		
		try {
			// insert the entry
			EricssonCorporationContextListener.mongoCollection.insert(nextEntry,
	...
	public static EricssonCorporationEntry getItem(int id) {
        ...
        if (cursor.hasNext()){
        	...
        	int itemId = (Integer)item.get("_id");
	...
	public static boolean deleteItem(int id) {
		try {
			DBObject query = BasicDBObjectBuilder.start().add("_id", id).get();
	        EricssonCorporationContextListener.mongoCollection.remove(query);
	        return true;
		} catch (WriteConcernException e) {
			e.printStackTrace();
			return false;
